...
export function PropertyMediaLibrary({
  ...
  function toggleAsset(mediaAssetId: string) {
    ...
    if (selectedAssetIds.includes(mediaAssetId)) {
      onChange(selectedAssetIds.filter((id) => id !== mediaAssetId));
      return;
    }

    onChange([...selectedAssetIds, mediaAssetId]);
  }
  ...
  async function handleFileChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      onChange([...selectedAssetIds, ...uploadedIds.filter((id) => !selectedAssetIds.includes(id))]);
      setMessage({
        type: "success",
  ...
  return (
    <div className="stack" style={{ gap: "0.85rem" }}>
      ...
      >
        <div>
          <p style={{ margin: 0, fontWeight: 700, color: "var(--text)" }}>Media Library</p>
          <p style={{ margin: "0.2rem 0 0", fontSize: "0.82rem", color: "var(--text-muted)" }}>
            Upload photos once, then select the images that should appear on this property.
          </p>
        </div>
      ...
      >
        <span>{selectedAssetIds.length} image{selectedAssetIds.length === 1 ? "" : "s"} selected</span>
