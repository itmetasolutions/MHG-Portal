...
export async function POST(request: NextRequest, { params }: Params) {
  ...
  if (room.property.vacancyType !== "MULTIPLE") {
    return NextResponse.json(
      { error: "INVALID_VACANCY_TYPE", message: "Room close is only valid for shared properties." },
      { status: 400 },
    );
  }
  ...
  if (!["AVAILABLE", "DRAFT"].includes(room.property.status)) {
    return NextResponse.json({ error: "INVALID_PROPERTY_STATUS", message: "Property must be AVAILABLE or DRAFT." }, { status: 400 });
  }
  ...
  const result = await db.$transaction(async (tx) => {
    const sale = await tx.sale.create({
      ...
      select: {
        id: true, propertyId: true, roomId: true, finalAmount: true,
    ...
    await tx.propertyRoom.update({ where: { id: room.id }, data: { status: "CLOSED" } });
    ...
    if (openRoomsCount === 0) {
      await tx.property.update({ where: { id: room.propertyId }, data: { status: "CLOSED" } });
    }

    await tx.landlord.updateMany({ where: { id: room.property.landlord.id, isPassive: true }, data: { isPassive: false, passiveMarkedAt: null } });
    ...
    await tx.auditLog.create({
      data: {
        ...
        metadata: { roomId: room.id, roomName: room.roomName, landlordId: room.property.landlordId, finalRent, companyCommission, agentCommissionAmt, otherCosts, profit, tenantId: tenant.id, allRoomsClosed: openRoomsCount === 0 },
